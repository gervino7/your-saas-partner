...
const STATUS_LABELS: Record<string, string> = { scheduled: 'Programmée', in_progress: 'En cours', completed: 'Terminée', cancelled: 'Annulée' };
const STATUS_COLORS: Record<string, string> = { scheduled: 'bg-info/15 text-info', in_progress: 'bg-warning/15 text-warning', completed: 'bg-success/15 text-success', cancelled: 'bg-destructive/15 text-destructive' };
...
function DecisionsEditor({ meetingId, committeeId, initialDecisions, canManage }: { meetingId: string; committeeId: string; initialDecisions: Decision[]; canManage: boolean; }) {
  ...
  const handleChange = (idx: number, field: string, value: string) => {
    setDecisions(prev => { const updated = [...prev]; updated[idx] = { ...updated[idx], [field]: value }; return updated; });
    setDirty(true);
  };
  const handleAdd = () => { setDecisions(prev => [...prev, { text: '', responsible: '', deadline: '', status: 'todo' }]); setDirty(true); };
  const handleSave = () => { updateMeeting.mutate({ id: meetingId, committee_id: committeeId, decisions: decisions as unknown as Json }, { onSuccess: () => setDirty(false) }); };
    ...
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="flex gap-2">
          {canManage && dirty && (<Button size="sm" variant="default" onClick={handleSave} disabled={updateMeeting.isPending}><Save className="h-3 w-3 mr-1" />Enregistrer</Button>)}
          {canManage && (<Button size="sm" variant="outline" onClick={handleAdd}><Plus className="h-3 w-3 mr-1" />Décision</Button>)}
        </div>
      </div>
      {decisions.map((d, i) => (
        <div key={i} className="border border-border/50 rounded-md p-3 mb-2 space-y-2">
          <Input placeholder="Décision..." value={d.text} onChange={(e) => handleChange(i, 'text', e.target.value)} disabled={!canManage} />
...
const MeetingsSection = ({ committeeId, canManage }: Props) => {
  ...
  const [form, setForm] = useState({ title: '', agenda: '', scheduled_at: '', duration_minutes: '60', location: '', meeting_link: '' });

  const handleCreate = async () => {
    await createMeeting.mutateAsync({ committee_id: committeeId, title: form.title, agenda: form.agenda, scheduled_at: new Date(form.scheduled_at).toISOString(), duration_minutes: parseInt(form.duration_minutes) || 60, location: form.location, meeting_link: form.meeting_link });
    setOpen(false);
    setForm({ title: '', agenda: '', scheduled_at: '', duration_minutes: '60', location: '', meeting_link: '' });
  };

  const handleStatusChange = (meetingId: string, status: string) => { updateMeeting.mutate({ id: meetingId, committee_id: committeeId, status }); };
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-3">
            ...
            <Collapsible key={m.id}>
              <div className="border border-border/40 rounded-lg p-4">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="text-left">
                        ...
                        <p className="text-sm text-muted-foreground">{format(new Date(m.scheduled_at), 'PPP à HH:mm', { locale: fr })}{m.duration_minutes && ` · ${m.duration_minutes} min`}</p>
                      </div>
                    </div>
                ...
                <CollapsibleContent className="mt-4 space-y-4">
                  {m.agenda && (<div><p className="text-sm font-medium mb-1">Ordre du jour</p><p className="text-sm text-muted-foreground whitespace-pre-wrap">{m.agenda}</p></div>)}
                  {(m.location || m.meeting_link) && (
                    <div className="flex gap-4 text-sm">
                      ...
                      {m.meeting_link && (<a href={m.meeting_link} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1 text-primary hover:underline"><Video className="h-3 w-3" />Rejoindre</a>)}
                    </div>
                  )}
                  {m.status === 'completed' && (<DecisionsEditor meetingId={m.id} committeeId={committeeId} initialDecisions={decisions} canManage={canManage} />)}
                  {canManage && (
                    <div className="flex gap-2 pt-2 border-t">
                      ...
                      {(m.status === 'scheduled' || m.status === 'in_progress') && (<Button size="sm" variant="ghost" onClick={() => handleStatusChange(m.id, 'cancelled')}>Annuler</Button>)}
                    </div>
                  )}
                </CollapsibleContent>
              </div>
        ...
        {(!meetings || meetings.length === 0) && (<p className="text-center text-muted-foreground py-8">Aucune réunion programmée</p>)}
      </CardContent>
    </Card>
  );
};
